...
void BTree<Key, Value>::insert(const Key &key, const Value &value)
{
    ...
    if (static_cast<int>(r->keys.size()) == 2 * t - 1)
    {
        ...
        newRootPtr->children.push_back(move(root));
        root = move(newRoot);
...
void BTree<Key, Value>::splitChild(Node *parent, int idx)
{
    ...
    auto newChild = make_unique<Node>(fullChild->leaf);
    ...
    if (!fullChild->leaf)
    {
        for (auto it = fullChild->children.begin() + t; it != fullChild->children.end(); ++it)
            newChild->children.push_back(move(*it));
    ...
    parent->children.insert(parent->children.begin() + idx + 1, move(newChild));
...
void BTree<Key, Value>::removeFromNode(Node *node, const Key &key)
{
    ...
    if (node->leaf)
        return; // key not found (shouldn't happen, checked earlier)
...
void BTree<Key, Value>::borrowFromPrev(Node *node, int idx)
{
    ...
    if (!child->leaf)
        child->children.insert(child->children.begin(), move(sibling->children.back()));
...
void BTree<Key, Value>::borrowFromNext(Node *node, int idx)
{
    ...
    if (!child->leaf)
        child->children.push_back(move(sibling->children.front()));
...
void BTree<Key, Value>::merge(Node *node, int idx)
{
    ...
    for (auto &v : sibling->values)
        child->values.push_back(v);
    if (!child->leaf)
        for (auto &c : sibling->children)
            child->children.push_back(move(c));
...
// traverse
template <typename Key, typename Value>
void BTree<Key, Value>::inorder(const function<void(const Key &, const Value &)> &visit) const
{
...
void BTree<Key, Value>::inorderNode(Node *node,
                                    const function<void(const Key &, const Value &)> &visit) const
{
    if (!node)
        return;
    size_t i;
    ...
    {
        if (!node->leaf)
            inorderNode(node->children[i].get(), visit);
